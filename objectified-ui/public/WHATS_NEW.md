...
## Git Behavior
- **Push optimistic lock (#2566):** **REST** create revision (`POST /v1/versions/...`) requires **`baseRevisionId`** (and **`branchName`** when the project has multiple branches); wrong or missing base → **`400`**; stale head → **`409`** with **`STALE_HEAD`** and current head metadata; ADE **Commit** sends the branch tip as **`baseRevisionId`**
