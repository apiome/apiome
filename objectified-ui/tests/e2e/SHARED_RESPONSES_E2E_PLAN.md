...
-- Create test data
INSERT INTO odb.tenant (id, name) VALUES ('test-tenant-id', 'Test Tenant');
INSERT INTO odb.project (id, tenant_id, name) VALUES ('test-project-id', 'test-tenant-id', 'Test Project');
INSERT INTO odb.version (id, project_id, version) VALUES ('test-version-id', 'test-project-id', '1.0.0');
INSERT INTO odb.version_path (id, version_id, pathname) VALUES ('test-path-id', 'test-version-id', '/api/users/{id}');
...
**SQL Verification:**
```sql
SELECT * FROM odb.shared_path_response WHERE status_code = '200';
SELECT * FROM odb.path_operation_response_link WHERE shared_path_response_id = '<response-id>';
```
...
SELECT COUNT(*) FROM odb.shared_path_response WHERE status_code = '200';

-- Should return 2 rows (links to GET and POST)
SELECT COUNT(*) FROM odb.path_operation_response_link 
...
**SQL Verification:**
```sql
SELECT * FROM odb.path_operation_response_link 
...
SELECT COUNT(*) FROM odb.shared_path_response 
...
```bash
cd objectified-db
psql -U postgres -d objectified -f scripts/test_shared_path_responses_migration.sql
```
