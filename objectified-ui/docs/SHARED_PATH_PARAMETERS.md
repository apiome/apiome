...
```sql
CREATE TABLE odb.shared_path_parameter (
    id UUID PRIMARY KEY,
    version_path_id UUID NOT NULL REFERENCES odb.version_path(id),
...
```sql
CREATE TABLE odb.path_operation_parameter_link (
    id UUID PRIMARY KEY,
    path_operation_id UUID NOT NULL REFERENCES odb.path_operation(id),
    shared_path_parameter_id UUID NOT NULL REFERENCES odb.shared_path_parameter(id),
...
\i objectified-db/scripts/20260110-111100.sql

-- Verify migration
SELECT COUNT(*) FROM odb.shared_path_parameter;
SELECT COUNT(*) FROM odb.path_operation_parameter_link;
