/**
 * Single MCP endpoint — proxies to apiome-rest:
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ endpointId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: 'MCP endpoint API unavailable (apiome-rest not reachable).' },
      { status: 503 },
    );
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ endpointId: string }> }) {
  ...
  // mutable here; apiome-rest re-validates each (transport/visibility enums, URL scheme,
