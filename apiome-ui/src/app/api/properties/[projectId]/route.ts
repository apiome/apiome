/**
 * API Proxy for Properties Management by Project
 *
 * Proxies requests to the REST API with JWT authentication.
 * Handles GET (list) and POST (create) operations for properties in a project.
 */
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${REST_API_BASE_URL}/properties/${tenantSlug}/${projectId}`, {
      method: 'GET',
      headers,
    });

    const { data, error, status } = await handleRestResponse(response, 'Failed to fetch properties');
...
export async function POST(
  ...
  try {
    ...
    const { data, error, status } = await handleRestResponse(response, 'Failed to create property');
