/**
 * API Proxy for Primitive Import from JSON Schema
 *
 * Proxies requests to the REST API with JWT authentication.
 * Handles importing primitives from JSON Schema $defs.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${REST_API_BASE_URL}/primitives/${tenantSlug}/import`,
      {
        method: 'POST',
        headers,
