...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${REST_API_BASE_URL}/identity/${ctx.tenantSlug}/projects/${encodeURIComponent(projectId)}/related`,
      { method: 'GET', headers: ctx.headers },
    );
    const { data, error, status } = await handleIdentityRestResponse(
      response,
      'Failed to fetch related artifacts',
    );
    if (error) {
      return NextResponse.json({ success: false, error }, { status });
    }
    return NextResponse.json({ success: true, relatedArtifacts: data });
  } catch (error) {
