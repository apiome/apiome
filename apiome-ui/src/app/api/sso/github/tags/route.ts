...
/** List tags (first page, up to 100). */
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `https://api.github.com/repos/${encodeURIComponent(repoOwner)}/${encodeURIComponent(repoName)}/tags?per_page=100`;
    ...
    const tags: unknown = await githubResponse.json();
    const names = Array.isArray(tags)
      ? tags
          .map((t) => (typeof t === 'object' && t !== null && 'name' in t ? String((t as { name: string }).name) : ''))
          .filter(Boolean)
      : [];
