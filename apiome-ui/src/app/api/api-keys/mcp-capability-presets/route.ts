/**
 * MCP capability presets proxy — MTG-5.1 (#4785).
 *
 * Forwards to apiome-rest `GET /api-keys/mcp-capability-presets` so the
 * Tenants MCP Settings panel can apply named toolset matrices to the draft
 * policy. The catalog lives at the API root (not under `/v1`).
 */
...
export async function GET() {
  ...
  const url = `${restApiOrigin()}/api-keys/mcp-capability-presets`;
  try {
