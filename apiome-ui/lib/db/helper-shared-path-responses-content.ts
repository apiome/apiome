...
export async function updateResponseContentType(
  ...
  try {
    ...
    const query = `
      UPDATE apiome.shared_path_response_content
      SET ${setParts.join(', ')}
      WHERE id = $${paramIndex}
      RETURNING id, shared_path_response_id, media_type, class_id, inline_schema, examples
    `;
...
export async function deleteResponseContentType(contentId: string): Promise<string> {
  try {
    const query = `DELETE FROM apiome.shared_path_response_content WHERE id = $1`;
  ...
  try {
    const query = `
      SELECT 
        ...
        rc.inline_schema,
        rc.examples
      FROM apiome.shared_path_response_content rc
      LEFT JOIN apiome.classes c ON rc.class_id = c.id
...
export async function convertResponseClassToInlineSchema(contentId: string): Promise<string> {
  try {
    ...
    const getQuery = `
      ...
      FROM apiome.shared_path_response_content rc
      JOIN apiome.classes c ON rc.class_id = c.id
      WHERE rc.id = $1
    `;
...
export async function addPropertyToResponseInlineSchema(
  ...
  try {
    ...
    const getQuery = `SELECT inline_schema FROM apiome.shared_path_response_content WHERE id = $1`;
    const getResult = await connectionPool.query(getQuery, [contentId]);
