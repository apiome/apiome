...
export async function createUser(
  ...
  try {
    ...
    const existingUser = await connectionPool.query(
      'SELECT id FROM apiome.users WHERE email = $1',
      [email]
    );
...
export async function createUserFromSignup(
  ...
  try {
    ...
    const signupResult = await connectionPool.query(
      'SELECT name, email_address, password, signup_source FROM apiome.signup WHERE email_address = $1',
      [email]
    );
    ...
    await connectionPool.query(
      'DELETE FROM apiome.signup WHERE email_address = $1',
      [email]
    );
...
export async function deleteSignup(email: string) {
  try {
    const result = await connectionPool.query(
      'DELETE FROM apiome.signup WHERE email_address = $1 RETURNING email_address',
      [email]
    );
  ...
  try {
    const result = await connectionPool.query(
      `SELECT 
       ...
       LEFT JOIN apiome.tenant_users tu ON t.id = tu.tenant_id
       LEFT JOIN apiome.tenant_administrators ta ON t.id = ta.tenant_id
       LEFT JOIN apiome.projects p ON t.id = p.tenant_id AND p.deleted_at IS NULL
  ...
  try {
    const result = await connectionPool.query(
      `SELECT 
        ...
        EXISTS(SELECT 1 FROM apiome.tenant_administrators ta WHERE ta.tenant_id = $1 AND ta.user_id = u.id) as is_admin
       FROM apiome.users u
       JOIN apiome.tenant_users tu ON u.id = tu.user_id
...
export async function createTenant(
  ...
  try {
    ...
    const existingTenant = await connectionPool.query(
      'SELECT id FROM apiome.tenants WHERE slug = $1 AND deleted_at IS NULL',
      [slug]
    );
...
export async function updateTenant(
  ...
  try {
    ...
    if (updates.slug !== undefined) {
      ...
      const existingTenant = await connectionPool.query(
        'SELECT id FROM apiome.tenants WHERE slug = $1 AND id != $2 AND deleted_at IS NULL',
...
export async function addUserToTenant(tenantId: string, userId: string) {
  try {
    ...
    const tenantCheck = await connectionPool.query(
      'SELECT id FROM apiome.tenants WHERE id = $1 AND deleted_at IS NULL',
      [tenantId]
    );
    ...
    const userCheck = await connectionPool.query(
      'SELECT id FROM apiome.users WHERE id = $1 AND deleted_at IS NULL',
      [userId]
    );
    ...
    const existingAssignment = await connectionPool.query(
      'SELECT id FROM apiome.tenant_users WHERE tenant_id = $1 AND user_id = $2',
      [tenantId, userId]
    );
...
export async function removeUserFromTenant(tenantId: string, userId: string) {
  try {
    ...
    await connectionPool.query(
      'DELETE FROM apiome.tenant_administrators WHERE tenant_id = $1 AND user_id = $2',
      [tenantId, userId]
    );

    const result = await connectionPool.query(
      'DELETE FROM apiome.tenant_users WHERE tenant_id = $1 AND user_id = $2 RETURNING user_id',
      [tenantId, userId]
    );
...
export async function addTenantAdministrator(tenantId: string, userId: string) {
  try {
    ...
    const existingAdmin = await connectionPool.query(
      'SELECT id FROM apiome.tenant_administrators WHERE tenant_id = $1 AND user_id = $2',
      [tenantId, userId]
    );
 ...
 * `apiome.provision_sample_project()` routine (apiome-db migration V122), owned by `creatorId`.
...
export async function provisionSampleProject(tenantId: string, creatorId: string) {
  try {
    const result = await connectionPool.query(
      'SELECT apiome.provision_sample_project($1, $2) AS project_id',
      [tenantId, creatorId]
    );
...
export async function removeTenantAdministrator(tenantId: string, userId: string) {
  try {
    const result = await connectionPool.query(
      'DELETE FROM apiome.tenant_administrators WHERE tenant_id = $1 AND user_id = $2 RETURNING user_id',
      [tenantId, userId]
    );
  ...
  try {
    const result = await connectionPool.query(
      `SELECT id, name, email, verified, enabled
       ...
       WHERE deleted_at IS NULL
         AND id NOT IN (
           SELECT user_id FROM apiome.tenant_users WHERE tenant_id = $1
         )
       ORDER BY name`,
      [tenantId]
    );
  ...
  try {
    const result = await connectionPool.query(
      `SELECT pt.id, pt.name, pt.description, pt.category, pt.schema, pt.tags,
       ...
       FROM apiome.property_templates pt
       LEFT JOIN apiome.tenants t ON pt.tenant_id = t.id
       LEFT JOIN apiome.users u ON pt.created_by = u.id
  ...
  try {
    const result = await connectionPool.query(
      `SELECT
       ...
       LEFT JOIN apiome.license_feature_flags lff ON lff.license_id = l.id
       LEFT JOIN apiome.feature_flags ff           ON ff.id = lff.feature_flag_id
       GROUP BY l.id
...
export async function createLicense(
  ...
  try {
    ...
    if (featureFlagIds.length > 0) {
      ...
      await connectionPool.query(
        `INSERT INTO apiome.license_feature_flags (license_id, feature_flag_id) VALUES ${placeholders}
...
export async function updateLicense(
  ...
  try {
    ...
    if (fields.length > 0) {
      ...
      const result = await connectionPool.query(
        `UPDATE apiome.licenses SET ${fields.join(', ')} WHERE id = $${idx} RETURNING id`,
        values
      );
      if (result.rowCount === 0) return errorResponse('License not found');
    }
    ...
    if (updates.seats !== undefined) {
      ...
      await connectionPool.query(
        `UPDATE apiome.user_entitlements
    ...
    if (updates.featureFlagIds !== undefined) {
      await connectionPool.query(
        `DELETE FROM apiome.license_feature_flags WHERE license_id = $1`, [licenseId]
      );
...
export async function deleteLicense(licenseId: string) {
  try {
    const assigned = await connectionPool.query(
      `SELECT COUNT(*)::int AS c FROM apiome.user_entitlements WHERE license_id = $1`,
      [licenseId]
    );
    ...
    const result = await connectionPool.query(
      `DELETE FROM apiome.licenses WHERE id = $1 RETURNING id, name`, [licenseId]
    );
    if (result.rowCount === 0) return errorResponse('License not found');
    return successResponse({ deleted: result.rows[0] });
  } catch (error: any) {
...
export async function createFeatureFlag(
  ...
  try {
    const result = await connectionPool.query(
      `INSERT INTO apiome.feature_flags (name, label, description, url_patterns, is_preview)
       VALUES ($1, $2, $3, $4, $5)
...
export async function updateFeatureFlag(
  ...
  try {
    ...
    const result = await connectionPool.query(
      `UPDATE apiome.feature_flags SET ${fields.join(', ')} WHERE id = $${idx}
...
export async function deleteFeatureFlag(flagId: string) {
  try {
    const result = await connectionPool.query(
      `DELETE FROM apiome.feature_flags WHERE id = $1 RETURNING id, name`, [flagId]
    );
    if (result.rowCount === 0) return errorResponse('Feature flag not found');
    return successResponse({ deleted: result.rows[0] });
  } catch (error: any) {
  ...
  try {
    const result = await connectionPool.query(
      `SELECT
       ...
       FROM apiome.feature_flag_groups g
       LEFT JOIN apiome.feature_flag_group_members m ON m.group_id = g.id
       LEFT JOIN apiome.feature_flags ff ON ff.id = m.feature_flag_id
       GROUP BY g.id
       ORDER BY g.label`
    );
...
export async function createFeatureFlagGroup(
  ...
  try {
    ...
    const insert = await client.query(
      `INSERT INTO apiome.feature_flag_groups (name, label, description)
       VALUES ($1, $2, $3)
    ...
    if (uniqueFlagIds.length > 0) {
      ...
      await client.query(
        `INSERT INTO apiome.feature_flag_group_members (group_id, feature_flag_id) VALUES ${values}`,
  ...
  try {
    ...
    if (fields.length > 0) {
      ...
      const res = await client.query(
        `UPDATE apiome.feature_flag_groups SET ${fields.join(', ')} WHERE id = $${idx} RETURNING id`,
        values
      );
      if (res.rowCount === 0) {
        await client.query('ROLLBACK');
      ...
      const check = await client.query(`SELECT id FROM apiome.feature_flag_groups WHERE id = $1`, [groupId]);
      if (check.rowCount === 0) {
        await client.query('ROLLBACK');
        return errorResponse('Feature flag package not found');
      }
    ...
    if (updates.featureFlagIds !== undefined) {
      await client.query(`DELETE FROM apiome.feature_flag_group_members WHERE group_id = $1`, [groupId]);
      const uniqueIds = [...new Set(updates.featureFlagIds)];
      if (uniqueIds.length > 0) {
        const placeholders = uniqueIds.map((_, i) => `($1, $${i + 2})`).join(', ');
        await client.query(
          `INSERT INTO apiome.feature_flag_group_members (group_id, feature_flag_id) VALUES ${placeholders}`,
          [groupId, ...uniqueIds]
        );
      }
    }
...
export async function deleteFeatureFlagGroup(groupId: string) {
  try {
    const result = await connectionPool.query(
      `DELETE FROM apiome.feature_flag_groups WHERE id = $1 RETURNING id, label`,
      [groupId]
    );
  ...
  try {
    const result = await connectionPool.query(
      `SELECT
         ...
         ue.plan_code
       FROM apiome.users u
       LEFT JOIN apiome.user_entitlements ue ON ue.user_id = u.id
       LEFT JOIN apiome.licenses          l  ON l.id = ue.license_id
      ...
      `SELECT
         ...
         COALESCE(
           (
             ...
             FROM apiome.license_feature_flags lff
             JOIN apiome.feature_flags ff ON ff.id = lff.feature_flag_id
         ...
         COALESCE(
           (
             ...
             FROM apiome.user_feature_flags uff
             JOIN apiome.feature_flags uff_ff ON uff_ff.id = uff.feature_flag_id
...
export async function assignLicenseToUser(userId: string, licenseId: string) {
  try {
    const licenseResult = await connectionPool.query(
      `SELECT license_type, seats FROM apiome.licenses WHERE id = $1`,
      [licenseId]
    );
...
export async function removeUserLicense(userId: string) {
  try {
    await connectionPool.query(
      `UPDATE apiome.user_entitlements SET license_id = NULL WHERE user_id = $1`, [userId]
    );
...
export async function setUserFeatureFlag(
  ...
  try {
    await connectionPool.query(
      `INSERT INTO apiome.user_feature_flags (user_id, feature_flag_id, enabled, granted_by)
       VALUES ($1, $2, $3, $4)
...
export async function removeUserFeatureFlag(userId: string, featureFlagId: string) {
  try {
    await connectionPool.query(
      `DELETE FROM apiome.user_feature_flags WHERE user_id = $1 AND feature_flag_id = $2`,
...
export async function setTenantFeatureFlag(
  ...
  try {
    await connectionPool.query(
      `INSERT INTO apiome.tenant_feature_flags (tenant_id, feature_flag_id, enabled, granted_by)
       VALUES ($1, $2, $3, $4)
...
export async function removeTenantFeatureFlag(tenantId: string, featureFlagId: string) {
  try {
    await connectionPool.query(
      `DELETE FROM apiome.tenant_feature_flags WHERE tenant_id = $1 AND feature_flag_id = $2`,
  ...
  try {
    const result = await connectionPool.query(
      `SELECT
       ...
       FROM apiome.tenant_feature_flags tff
       JOIN apiome.feature_flags ff ON ff.id = tff.feature_flag_id
