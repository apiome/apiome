...
export async function getLinkedResponsesForOperation(operationId: string): Promise<string> {
  const query = `
    ...
    FROM apiome.shared_path_response spr
    JOIN apiome.path_operation_response_link porl ON spr.id = porl.shared_path_response_id
    LEFT JOIN apiome.classes c ON spr.class_id = c.id
    LEFT JOIN apiome.shared_path_response_content rc ON spr.id = rc.shared_path_response_id
    LEFT JOIN apiome.classes rc_class ON rc.class_id = rc_class.id
...
export async function deleteSharedPathResponse(responseId: string): Promise<string> {
  ...
  const checkQuery = `
    ...
    FROM apiome.path_operation_response_link
  ...
  try {
    ...
    const query = 'DELETE FROM apiome.shared_path_response WHERE id = $1';
