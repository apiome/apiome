...
describe('catalog-format-registry — importable vs recognized (MFI-23.12)', () => {
  test('importable alternatives are exactly the adapter-backed (store-raw) formats', () => {
    ...
    expect(IMPORTABLE_ALTERNATIVE_FORMATS.map((f) => f.id).sort()).toEqual(
      ['asn1', 'asyncapi', 'avro', 'apiblueprint', 'capnproto', 'cloudevents', 'connectrpc', 'corbaidl', 'edix12', 'flatbuffers', 'fhir', 'graphql', 'grpc', 'hl7v2', 'iso20022', 'odata', 'oncrpc', 'openrpc', 'postman', 'protobuf', 'raml', 'smithy', 'thrift', 'typespec', 'wadl', 'wsdl', 'xmlrpc', 'xsd'].sort(),
    );
  });
