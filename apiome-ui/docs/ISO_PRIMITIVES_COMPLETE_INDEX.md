...
**File**: `apiome-db/scripts/20260124-140000.sql`  
**Size**: 21 KB  
...
**File**: `apiome-rest/test_iso_primitives_preload.py`  
**Size**: 8.5 KB  
...
```sql
SELECT name, category, description FROM apiome.primitives 
...
**Search by tag**:
```sql
SELECT name FROM apiome.primitives 
WHERE tags @> ARRAY['iso-standard'] AND is_system = true;
```

**Check tenant distribution**:
```sql
SELECT t.slug, COUNT(p.id) FROM apiome.tenants t
LEFT JOIN apiome.primitives p ON t.id = p.tenant_id AND p.is_system = true
