...
# Docker Build and Publish Script for Apiome Web
...
test_image() {
    ...
    local test_container="apiome-web-test-$$"
    docker run -d \
        --name "$test_container" \
        -p 3001:3002 \
...
# Deployment script for Apiome Web
...
docker stop apiome-web 2>/dev/null || true
docker rm apiome-web 2>/dev/null || true
...
echo ""
echo "Container logs:"
docker logs --tail 50 apiome-web
EOF
...
create_deployment_package() {
    ...
    local package_dir="apiome-web-deploy-${VERSION}"
...
Stop: \`docker stop apiome-web\`
Restart: \`docker restart apiome-web\`
EOF
...
# Main execution
main() {
    print_info "=== Apiome Web Docker Build Script ==="
    echo ""
    ...
    [ "$do_package" = true ] && echo "  4. Transfer: apiome-web-deploy-${VERSION}.tar.gz to remote server"
    echo ""
}
