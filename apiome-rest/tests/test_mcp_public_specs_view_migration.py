...
_MIGRATION_FRAGMENTS = (
    "CREATE OR REPLACE VIEW apiome.mcp_v_public_specs",
    "p.name AS title",
    ...
    "version_tags",
    "v.published IS TRUE",
    "v.visibility = 'public'::apiome.visibility_type",
    "COMMENT ON VIEW apiome.mcp_v_public_specs",
    "COMMENT ON COLUMN apiome.mcp_v_public_specs.tags",
...
_FIXTURE_FRAGMENTS = (
    "Dev fixture for apiome.mcp_v_public_specs",
    "SELECT count(*) FROM apiome.mcp_v_public_specs",
...
def test_migration_defines_mcp_v_public_specs(repo_root: Path) -> None:
    migration = repo_root / "apiome-db" / "scripts" / "V095__mcp_read_model_published_public_schema_r.sql"
...
def test_dev_fixture_documents_public_specs_view(repo_root: Path) -> None:
    fixture = repo_root / "apiome-db" / "fixtures" / "mcp_public_specs_dev.sql"
    ...
    with psycopg2.connect(_db_url) as conn:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                "WHERE table_schema = 'apiome' AND table_name = 'mcp_v_public_specs' "
