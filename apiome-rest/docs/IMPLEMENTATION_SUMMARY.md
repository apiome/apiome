# Apiome REST API - Implementation Summary

## Overview
A FastAPI-based REST service that exposes OpenAPI specifications stored in the Apiome PostgreSQL database. The service provides versioned access to API specifications with support for both full version specs and individual class specs in JSON and YAML formats.
...
```bash
cd /home/kenji/Development/apiome/apiome-rest
...
The API requires these tables in the `apiome` schema:
- `apiome.tenants` - Tenant information with `slug`
- `apiome.projects` - Projects with `slug` and `tenant_id`
- `apiome.versions` - Versions with `version_id`, `visibility`, `published`
- `apiome.classes` - Classes with `name`, `description`, `schema`
- `apiome.class_properties` - Properties with `name`, `description`, `data`
