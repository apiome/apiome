# Public client kit (SDK-3.3, #4493; SDK-2.4, #4488)
...
- `app.sdk_kit` — pure builder (no FastAPI/db imports): what an archive contains and how it is
  named. Reuses `app.snippet_render` for the code, `app.go_client_generator` for the Go module,
  and `app.zip_bundle` for the bytes.
- `app.sdk_kit_routes` — the two HTTP surfaces described below.

## What the download actually is

**Built, never stored.** The original SDK-3.3 scope served "the latest generated artifact" from
the SDK-1.1 artifact store. SDK-1.1 (#4481), the generator SPI (#4482), both MVP language
generators (#4485/#4486) and the dashboard/CLI surfaces (#4491/#4492) were all closed
**not-planned**, so there is no artifact store — and nothing to retain, expire, invalidate or
garbage-collect.

What *did* ship reads the persisted canonical model directly, and the kit packages both halves:

- **SDK-2.3 snippets** — one runnable call per operation in `ts`, `python` and `curl`;
- **an SDK-2.4 Go client** — a complete, dependency-free Go module under `go/`, generated by
  `app.go_client_generator` (see `go_client_generator.md`).
...
├── go/                          a compilable Go client module (SDK-2.4)
│   ├── go.mod                   module path from the `gomod` branding, else example.com/…
│   ├── client.go                net/http transport behind an injectable Doer
│   ├── models.go                one exported Go type per schema
│   ├── errors.go                *APIError plus a type per declared error response
│   ├── auth.go                  one option per declared security scheme, when any
│   ├── <group>.go               context-first methods, one file per operation group
│   ├── examples/<group>/main.go a runnable example per group
│   └── README.md                go get, quick start, auth, errors, operations
└── snippets/
    ├── ts/getWidget.ts
    ├── python/getWidget.py
    └── curl/getWidget.sh
...
Everything is built **per request**.
...
the command that installs each), the **`go_client`** block (directory, module path, package name,
Go version, `go get` command, method count), the operation counts, the licence header, the merged
settings' fingerprint, and the download's filename.

It **counts and names** rather than builds — `app.sdk_kit.summarize_kit` walks the model applying
the same eligibility rule the builder does (an operation needs an HTTP method and path), and
`app.sdk_kit.go_client_coordinates` derives the module path from the coordinates and the branding
alone. Neither renders a snippet or emits a line of Go for a call that would throw the result
away.
...
- the Go module is a pure function of the model and the branding — no clock, no environment, and
  every name handed out by one deterministic allocator.
...
| `renderer` | The renderer and kit schema that produced the snippets |
| `go_generator` | The generator and schema that produced the Go module |
...
- **`MAX_KIT_OPERATIONS` (250)** bounds the work, because the kit is built per request on an
  ...
  says so.
- **A Go generator failure never takes the download down.** `app.sdk_kit.plan_go_client` catches
  it, the archive ships its snippets alone, and both the manifest's `go_client.included` and the
  README say why there is no module.
- **The manifest's `go_client` block** records the module path, package name, Go version, method
  and type counts, auth options, example groups, one row per generated method, and the operations
  the generator skipped. `included: false` distinguishes "this revision could not produce one"
  from a kit predating SDK-2.4.
