"""Apiome type-definition bundle importer for the Primitives import pipeline (#3462).
...
source kind: an *Apiome type-definition bundle* — many interlinked types shipped
...
* captures the type's **inter-type** ``$ref`` edges — refs at a sibling type in the same
  ...
  ``{relative_ref, resolved_target, status}`` shape persisted on ``apiome.primitives.refs``,
...
# ``types`` is the Apiome bundle keyword; ``$defs`` / ``definitions`` are accepted as
...
def build_bundle_internal_ref_edges(schema: Any) -> List[Dict[str, str]]:
    ...
    ``apiome.primitives.refs``. ``resolved_target`` is the referenced type's local name (what
...
def parse_type_def_bundle(
    ...
    """Expand an Apiome type-definition bundle into discrete interlinked types (#3462).
