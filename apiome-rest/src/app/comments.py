...
primary key, never by where the element happens to sit on a canvas. A thread is ``open`` or
``resolved``. Its **comments** are Markdown replies, each recording the tenant members its
``@name`` tokens resolved to (see :mod:`app.comment_mentions`).
...
(:mod:`app.comment_store`) share. The storage lives in apiome-db V259.
"""
...
#: Thread states. Mirrors ``comment_threads_status_check``.
STATUS_OPEN = "open"
STATUS_RESOLVED = "resolved"
THREAD_STATUSES = (STATUS_OPEN, STATUS_RESOLVED)
...
class CommentThreadRecord(BaseModel):
    ...
    anchor_id: str = Field(
        description=(
            "The anchored element's stable id. Equals `version_id` for a version anchor."
        )
    )
    status: ThreadStatus
