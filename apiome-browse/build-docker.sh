...
# Docker Build and Publish Script for Apiome Browse
...
test_image() {
    ...
    local test_container="apiome-browse-test-$$"
    docker run -d \
        --name "$test_container" \
        -p 3001:3000 \
...
# Deployment script for Apiome Browse
...
docker stop apiome-browse 2>/dev/null || true
docker rm apiome-browse 2>/dev/null || true
...
echo ""
echo "Container logs:"
docker logs --tail 50 apiome-browse
EOF
...
create_deployment_package() {
    ...
    local package_dir="apiome-browse-deploy-${VERSION}"
...
# Apiome Browse Deployment Package
...
Check logs: \`docker logs apiome-browse\`
Stop: \`docker stop apiome-browse\`
Restart: \`docker restart apiome-browse\`
EOF
...
# Main execution
main() {
    print_info "=== Apiome Browse Docker Build Script ==="
    echo ""
    ...
    [ "$do_package" = true ] && echo "  4. Transfer: apiome-browse-deploy-${VERSION}.tar.gz to remote server"
    echo ""
}
