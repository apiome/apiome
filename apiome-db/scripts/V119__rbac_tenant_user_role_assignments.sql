...
-- legacy apiome.tenant_administrators table is retained as the authoritative "full access" signal for
-- backward compatibility — apiome-rest's guard treats any tenant_administrators row as Owner.
--
...
JOIN apiome.roles r ON r.tenant_id = ta.tenant_id AND r.slug = 'owner'
...
JOIN apiome.roles r ON r.tenant_id = tu.tenant_id AND r.slug = 'editor'
WHERE NOT EXISTS (
