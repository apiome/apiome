...
ALTER TABLE apiome.shared_path_response_content
DROP CONSTRAINT IF EXISTS shared_path_response_content_check;

-- Update comment to reflect the new behavior
COMMENT ON TABLE apiome.shared_path_response_content IS 'Content type variants for response bodies (supports multiple media types per response). Allows incremental inline schema building - both class_id and inline_schema can be NULL initially.';
