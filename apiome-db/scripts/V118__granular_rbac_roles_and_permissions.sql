...
-- role; the absence of a row means denied. apiome-rest's central permission guard reads this
-- grid on every mutating route (see apiome-rest/src/app/permissions.py).
--
...
-- existing tenant by this migration and on-demand by apiome-rest when a tenant's roles are
...
COMMENT ON FUNCTION apiome.seed_builtin_roles(UUID) IS 'Idempotently (re)seed the four built-in roles and their canonical permission grids for a tenant (#3611)';
