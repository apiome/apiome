...
-- tenant-create panel, and the apiome-db CLI. It is idempotent (a no-op if the sample already
...
-- live from classes/properties; see apiome-rest GET /v1/schema/...). It also writes the frozen
-- apiome.class_schema rows so the sample behaves exactly like a UI-published version.

SET search_path TO apiome, public;

CREATE OR REPLACE FUNCTION apiome.provision_sample_project(p_tenant_id uuid, p_creator_id uuid)
RETURNS uuid
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  -- Project
  INSERT INTO apiome.projects (tenant_id, creator_id, name, description, slug, enabled)
  VALUES (
    p_tenant_id, p_creator_id, 'Pet Store (sample)',
    'A curated sample API to explore Apiome: classes, a published version, and a browsable spec.',
  ...
  INSERT INTO apiome.properties (project_id, name, description, data, enabled)
  ...
  INSERT INTO apiome.class_properties (class_id, property_id, name, description, data) VALUES
  ...
  INSERT INTO apiome.class_schema (version_id, class_id, schema) VALUES
...
COMMENT ON FUNCTION apiome.provision_sample_project(uuid, uuid) IS
  'Idempotently provision the curated "Pet Store" sample project (published + public) for a tenant, owned by the given creator. Returns the new project id, or NULL if the sample already exists for the tenant. Shared by the dev seed, OAuth signup, admin tenant-create, and the apiome-db CLI.';
