...
CREATE TABLE IF NOT EXISTS apiome.shared_path_parameter (
...
CREATE TABLE IF NOT EXISTS apiome.path_operation_parameter_link (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    path_operation_id UUID NOT NULL REFERENCES apiome.path_operation(id) ON DELETE CASCADE,
    shared_path_parameter_id UUID NOT NULL REFERENCES apiome.shared_path_parameter(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_shared_path_parameter_version_path
    ON apiome.shared_path_parameter(version_path_id);

CREATE INDEX IF NOT EXISTS idx_shared_path_parameter_name
    ON apiome.shared_path_parameter(name);

CREATE INDEX IF NOT EXISTS idx_path_operation_parameter_link_operation
    ON apiome.path_operation_parameter_link(path_operation_id);

CREATE INDEX IF NOT EXISTS idx_path_operation_parameter_link_parameter
    ON apiome.path_operation_parameter_link(shared_path_parameter_id);
...
CREATE TRIGGER trigger_update_shared_path_parameter_updated_at
    BEFORE UPDATE ON apiome.shared_path_parameter
    FOR EACH ROW
...
CREATE TRIGGER trigger_update_path_operation_parameter_link_updated_at
    BEFORE UPDATE ON apiome.path_operation_parameter_link
    FOR EACH ROW
    EXECUTE FUNCTION update_shared_path_parameter_updated_at();
...
BEGIN
    ...
    FOR param_record IN
        ...
        FROM apiome.path_parameter pp
        INNER JOIN apiome.path_operation po ON pp.path_operation_id = po.id
    LOOP
