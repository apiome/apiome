...
CREATE TABLE IF NOT EXISTS apiome.tenant_repository_refresh_jobs (
  ...
  import_spec_id UUID REFERENCES apiome.repository_import_spec(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_tenant_repo_refresh_jobs_queued
  ON apiome.tenant_repository_refresh_jobs (created_at)
  WHERE status = 'queued';

CREATE INDEX IF NOT EXISTS idx_tenant_repo_refresh_jobs_repository
  ON apiome.tenant_repository_refresh_jobs (repository_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_tenant_repo_refresh_jobs_active_lineage
  ON apiome.tenant_repository_refresh_jobs (repository_id, branch, path)
  WHERE status IN ('queued', 'running');

COMMENT ON TABLE apiome.tenant_repository_refresh_jobs IS
...
COMMENT ON COLUMN apiome.tenant_repository_refresh_jobs.options_json IS
...
COMMENT ON COLUMN apiome.tenant_repository_refresh_jobs.refresh_reason IS
