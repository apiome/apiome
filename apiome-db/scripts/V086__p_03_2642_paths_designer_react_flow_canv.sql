...
-- ROLLBACK: DROP TABLE IF EXISTS apiome.version_path_canvas;

SET search_path TO apiome, public;

CREATE TABLE IF NOT EXISTS version_path_canvas (
  version_path_id UUID NOT NULL PRIMARY KEY REFERENCES apiome.version_path(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_version_path_canvas_updated_at
  ON apiome.version_path_canvas(updated_at);

COMMENT ON TABLE apiome.version_path_canvas IS
  'Paths tab React Flow layout JSON per version_path; semantic paths/ops live in version_path / path_operation (#2642)';

COMMENT ON COLUMN apiome.version_path_canvas.canvas IS
