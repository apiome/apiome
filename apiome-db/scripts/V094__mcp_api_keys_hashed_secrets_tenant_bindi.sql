...
CREATE INDEX IF NOT EXISTS idx_mcp_api_keys_prefix ON apiome.mcp_api_keys (prefix);

COMMENT ON TABLE apiome.mcp_api_keys IS
  'MCP authentication keys (#2997); column and constraint comments document invariants.';

COMMENT ON COLUMN apiome.mcp_api_keys.key_hash IS
  'Bcrypt (or equivalent) hash of the full secret; never store plaintext.';

COMMENT ON COLUMN apiome.mcp_api_keys.prefix IS
  'Non-secret prefix copied from the issued key for indexed lookup before hash verification.';

COMMENT ON COLUMN apiome.mcp_api_keys.scope_json IS
  'Read scope JSON: {"tenants":[uuid,...],"projects":[uuid,...]}. Empty lists = public at that dimension; see apiome_mcp.scope.Scope (#3001).';

COMMENT ON COLUMN apiome.mcp_api_keys.revoked_at IS
  'When set, the key must be rejected regardless of expires_at.';

COMMENT ON CONSTRAINT chk_mcp_api_keys_expires_after_created ON apiome.mcp_api_keys IS
  'expires_at must be NULL or on/after created_at.';

COMMENT ON CONSTRAINT chk_mcp_api_keys_revoked_after_created ON apiome.mcp_api_keys IS
