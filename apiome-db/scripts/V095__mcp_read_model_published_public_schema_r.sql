...
CREATE OR REPLACE VIEW apiome.mcp_v_public_specs AS
SELECT
  v.id,
  p.tenant_id,
  v.project_id,
...
COMMENT ON VIEW apiome.mcp_v_public_specs IS
  'MCP spec discovery: one row per schema revision (versions.id) that is published and public-visible (#3004).';

COMMENT ON COLUMN apiome.mcp_v_public_specs.id IS
  'Schema revision id (versions.id); stable spec identity for MCP tools.';

COMMENT ON COLUMN apiome.mcp_v_public_specs.tenant_id IS
  'Owning tenant (projects.tenant_id).';

COMMENT ON COLUMN apiome.mcp_v_public_specs.project_id IS
  'Project id (versions.project_id).';

COMMENT ON COLUMN apiome.mcp_v_public_specs.title IS
  'Human-readable project title (projects.name).';

COMMENT ON COLUMN apiome.mcp_v_public_specs.version IS
  'Semantic version label (versions.version_id).';

COMMENT ON COLUMN apiome.mcp_v_public_specs.description IS
  'Revision description (versions.description); nullable.';

COMMENT ON COLUMN apiome.mcp_v_public_specs.tags IS
  'Sorted distinct git-like tag names (version_tags.name) pointing at this revision; empty array if none.';

COMMENT ON COLUMN apiome.mcp_v_public_specs.updated_at IS
