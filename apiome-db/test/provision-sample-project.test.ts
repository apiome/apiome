 ...
 *
 * V122 adds the shared `apiome.provision_sample_project()` routine that seeds a curated, published,
...
describe("sample-project provisioning migration", () => {
  ...
  it("defines the shared provisioning function (rerun-safe)", () => {
    expect(lower).toMatch(/create or replace function apiome\.provision_sample_project\(/);
  ...
  it("is idempotent: skips when the sample already exists for the tenant", () => {
    expect(lower).toMatch(/if exists \(\s*select 1 from apiome\.projects/);
