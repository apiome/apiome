 ...
 * Registry surface in apiome-rest) and bundles it into the Paid and Sponsor license
...
describe("primitives-registry entitlement seed migration", () => {
  ...
  it("seeds the primitives-registry feature flag into apiome.feature_flags", () => {
    expect(sql).toMatch(/insert into apiome\.feature_flags/);
  ...
  it("bundles the flag into the Paid and Sponsor plans but not Free", () => {
    expect(sql).toMatch(/insert into apiome\.license_feature_flags/);
