#!/usr/bin/env node
/**
 * apiome-db — direct-to-database admin CLI.
 *
...
program
  .name("apiome-db")
  .description("Direct-to-database admin CLI for Apiome (bypasses REST; needs DB credentials).")
  .version("0.1.0")
  .showHelpAfterError();

// Connection + output globals (each also reads env: APIOME_DB_URL/DATABASE_URL or POSTGRES_*).
program
  .option("--database-url <url>", "Postgres connection string (env: APIOME_DB_URL / DATABASE_URL)")
  ...
  .option("--database <database>", "DB name (env: POSTGRES_DB; default apiome)")
...
program
  .command("clean")
  .description("Drop the apiome schema and migration history (destructive; disabled by default)")
...
backupCmd
  ...
  .option("--out <dir>", "Backup directory (env: APIOME_BACKUP_DIR; default ./backups)")
...
backupCmd
  ...
  .option("--out <dir>", "Backup directory (env: APIOME_BACKUP_DIR; default ./backups)")
  .option("--offsite <dir>", "Also mirror the artifact to this off-site directory (env: APIOME_BACKUP_OFFSITE_DIR)")
  .option("--encrypt-key-file <path>", "32-byte AES-256 key file (env: APIOME_BACKUP_KEY)")
...
backupCmd
  ...
  .description("Restore a logical backup into a sandbox schema (PITR via --as-of); never touches apiome")
...
function reportError(err: unknown): void {
  if (err instanceof CliError) {
    note(`apiome-db: ${err.message}`);
    if (err.hint) note(`  hint: ${err.hint}`);
    return;
  }
  const message = err instanceof Error ? err.message : String(err);
  note(`apiome-db: ${message}`);
}
...
async function startRepl(): Promise<void> {
  ...
  await runInteractiveRepl({
    ...
    color,
    execute: executeLine,
    historyFile: join(homedir(), ".apiome-db", "history"),
  });
}
