 ...
 *
 * The data layer is event-sourced: `apiome.data_record` holds one immutable row per change
 * (`created` / `updated` / `deleted` / `restored`) with a per-record monotonic `record_sequence`,
 * and `apiome.data_snapshot` caches the current materialized state. To recover the state of the world
