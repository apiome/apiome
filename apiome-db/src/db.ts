 ...
 * credentials and we talk to the `apiome` schema straight over a connection. All table references
 * are schema-qualified (`apiome.*`) to match `apiome-rest`.
 */
...
export function buildPgConfig(opts: ConnectionOptions): pg.ClientConfig {
  const url = opts.databaseUrl ?? process.env.APIOME_DB_URL ?? process.env.DATABASE_URL;
  if (url && url.trim() !== "") {
  ...
  return {
    ...
    database: opts.database ?? process.env.POSTGRES_DB ?? "apiome",
  };
}
...
export function describeConnection(opts: ConnectionOptions): string {
  ...
  if (url && url.trim() !== "") {
    try {
      ...
      return `${u.hostname}:${u.port || "5432"}/${u.pathname.replace(/^\//, "") || "apiome"}`;
    } catch {
      return "(database url)";
    }
  }
...
export async function withClient<T>(
  ...
  try {
    ...
    throw new CliError(`Could not connect to the database (${describeConnection(opts)}): ${message}`, {
      hint: "Set APIOME_DB_URL / POSTGRES_* env vars or pass --database-url (or --host/--user/...).",
      exitCode: 2,
    });
  }
  try {
...
export async function resolveTenant(client: pg.Client, ref: string): Promise<TenantRow> {
  ...
  const query = isUuid(value)
    ? "SELECT id, slug, name FROM apiome.tenants WHERE id = $1 AND deleted_at IS NULL"
    : "SELECT id, slug, name FROM apiome.tenants WHERE slug = $1 AND deleted_at IS NULL";
