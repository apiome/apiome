...
| Emitter-registry export | `GET /v1/export/{tenant}/targets?artifact=&version=`, `POST /v1/export/{tenant}/preview` (fidelity only, no artifact), `POST /v1/export/{tenant}/document` (emit through the SPI → document bytes, JSON/YAML per `Accept`; API key + tenant scope) |
...
| `src/apiome_cli/commands/` | Typer subcommands (`auth`, `api-keys`, `integrations`, `config`, `doctor`, `health`, `projects`, `properties`, `schemas`, `types`, `tokens`, `versions`, `paths`, `operations`, `workflows`, `spec`, `import`, `export`, `convert`, `repos`, `mcp`) |
...
| `src/apiome_cli/client/export_registry.py` | Emitter-registry export client: targets discovery (`GET /export/targets`) + dry-run fidelity preview (`POST /export/preview`) for `export` (MFX-9.4) |
...
| `export` | Emitter-registry export — the inverse of `import` (MFX-9.4). `export openapi --project P --version V -o FILE` writes the OpenAPI document (via the browse reconstruction `GET /v1/schema/...`, the same source as `spec export`) and surfaces the emitter registry's fidelity report (`POST /v1/export/{tenant}/preview`, target `openapi`), **exiting non-zero on a lossy/types-only export unless `--force`** (the document is written either way); `--yaml`/`--accept` pick the serialization. `export asyncapi --project P --version V -o FILE` writes the AsyncAPI 3 document — its bytes come from the Emitter SPI (`POST /v1/export/{tenant}/document`) while the fidelity report still comes from `POST /export/preview`; a REST/RPC source reframes onto channels and exports *lossy* (non-zero exit unless `--force`). `export grpc --project P --version V -o FILE` writes proto3 `.proto` source (REST target `protobuf` on `/document` + `/preview`); a native gRPC source exports *lossless*, a REST/OpenAPI source exports *lossy* (non-zero exit unless `--force`). `export graphql --project P --version V -o FILE` writes GraphQL SDL (target `graphql` on `/document` + `/preview`); a native Graph source exports *lossless*, a REST/OpenAPI source exports *lossy* (non-zero exit unless `--force`). `export avro --project P --version V -o FILE` writes Avro `.avsc` JSON (target `avro` on `/document` + `/preview`); a native data-schema source exports *lossless*, a REST/OpenAPI source exports *types-only* (non-zero exit unless `--force`). `export targets --project P [--version V]` lists the registered emitters + per-source fidelity (`GET /v1/export/{tenant}/targets`). API key + tenant scope. OpenAPI bytes come from the browse reconstruction; every other target emits through `/export/document` |
