...
| API change check suite | `POST` / `GET /v1/tenants/{tenant_slug}/projects/{project}/versions/{version}/check-suite` (GNC-3.1: run / latest evaluation; `GET` allowlisted for `diff:read`/`lint:read` CI keys), `GET /v1/tenants/{tenant_slug}/projects/{project}/check-suite/runs/{id}` (drill-down). A re-run of unchanged inputs is a `200` replay; the verdict is in `run.state` (`pending`/`pass`/`fail`/`skipped`), never in the HTTP status |
...
| `src/apiome_cli/client/check_suite.py` | Pure helpers for `checks`: run request body, verdict → exit code, text rendering (GNC-3.1) |
...
| `checks` | API change check suite (GNC-3.1 / #4740). `run --project P --version V [--commit SHA] [--pr N] [--publish/--no-publish] [--format text\|json\|md]` evaluates the suite (`POST /v1/tenants/{slug}/projects/{project}/versions/{version}/check-suite`) — lint, breaking, consumers, contract, sdk, per the tenant's suite policy — records one verdict and reports it on the pull request when the version is bound; a re-run over unchanged inputs is a `200` replay of the same evaluation. `show --project P (--version V [--commit SHA] \| --run ID) [--format …]` reads the latest evaluation (`GET …/check-suite[?commit_sha=]`) or one by id (`GET /v1/tenants/{slug}/projects/{project}/check-suite/runs/{id}`) and warns on stderr when it is stale. `md` prints the run's `summary` (the pull-request markdown). Exit `0` pass/skipped, `7` failed, `8` pending, `1`/`2` transport/rejected. API key + tenant scope |
