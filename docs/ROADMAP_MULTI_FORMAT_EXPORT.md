...
### MFX-3.4 — Export job status/polling  ·  **#3847**  ·  ✅ **Done**
- **Status.** Extended the MFX-3.1 poll surface (`apiome-rest/src/app/export_job_engine.py`, `GET /v1/export/{tenant_slug}/jobs/{job_id}`) to make a **terminal** job self-describing for UI/CLI pollers. `ExportJobResult` gains `download_path` — the relative URL a poller dereferences to fetch the emitted artifact bytes (the `_download_path` seam onto the delivery epics, MFX-4.x); it is set once a real export completes and stays `null` for a dry-run (no artifact). `ExportJobStatus` gains a structured `error` (`ExportJobError` — `code`/`message`/`context`), the machine-readable twin of a job's terminal error event so a poller renders a failure and branches on the code (e.g. `TRANSCODE_CONFIRMATION_REQUIRED` → resubmit with `confirm`, vs `SOURCE_LOAD_FAILED`) without scraping the event log; `_fail` records both surfaces from one call, and a cancel (not a failure) carries neither `error` nor `result`. So a terminal state returns **download ref + report** (completed) **or structured error** (failed). Tests appended to `tests/test_export_job_engine.py` and `tests/test_export_job_routes.py`. apiome-rest 1.76.5 → 1.76.6.
...
- **Status.** A new `apiome-rest/src/app/export_validation.py` module (`validate_emitted_artifact(target_format, emit_result, *, api) → EmittedArtifactValidation`, `async`) is the per-format dispatcher that re-parses an emitted artifact through its matching MFI importer — **reusing, not rebuilding** the existing re-import paths: the OpenAPI / GraphQL / AsyncAPI round-trip modules (`round_trip_openapi` / `round_trip_graphql` / `round_trip_asyncapi`, MFX-9.3 / 13.5 / whose `valid` verdict *is* this check), `fastavro`'s `validate_avro_schema` for Avro, and `buf` (`compile_proto_descriptor_set`) for protobuf. Each collapses to a uniform `EmittedArtifactValidation` (`applicable` / `validated` / `valid` / `errors` / `detail`, with a `failed` gate = applicable **and** validated **and** not valid). The export job's validation stage (`export_job_engine._drive_export_job`) now runs it after emit and **fails the job** with a structured `EMITTED_ARTIFACT_INVALID` error carrying the parser detail (MFX-3.4) when a validator rejects the artifact — before packaging/delivery — emitting `ARTIFACT_VALIDATED` on success; a toolchain-backed validator whose tool is absent (`asyncapi-parser`, `buf`) reports `VALIDATION_SKIPPED` (a `warn`: a possibly-valid export is never failed for a check that could not run) and a target with no importer (the sample no-op) reports `VALIDATION_NOT_APPLICABLE`. The MFX-3.1 `validate_emitted_result` placeholder (which reported `VALIDATION_DEFERRED`) is replaced by `build_validation_events`. Tests in `tests/test_export_validation.py` (valid-passes / broken-caught for every format, plus skip / not-applicable / infrastructure-failure paths) and `tests/test_export_job_engine.py` (an invalid emit fails the job end to end). apiome-rest 1.78.0 → 1.79.0.
...
### MFX-5.3 — Validation gating & report  ·  **#3854**
- **Problem.** MFX-5.1 re-parses the emitted artifact but poll surfaces had no structured gate/report alongside the fidelity envelope — UIs could not branch on a single verdict or render parser detail without scraping the event log.
- **Solution / Scope.** A new `apiome-rest/src/app/export_validation_gate.py` module (`build_validation_report(validation) → EmittedValidationReport`) maps the MFX-5.1 `EmittedArtifactValidation` into four bands (`valid` / `invalid` / `skipped` / `not_applicable`) with `blocks_delivery` and `warns` gates, ready-to-render `headline` / `message` copy, tool identity per target, and structured `ValidationFinding` rows (message, path, file, line/column). `EmittedArtifactValidation` now carries `findings` alongside the legacy `errors` one-liners. Completed export jobs attach the report on `ExportJobResult.validation` (null for dry-runs); `EMITTED_ARTIFACT_INVALID` failures embed the full report in `error.context`. Tests in `tests/test_export_validation_gate.py` and MFX-5.3 cases in `tests/test_export_job_engine.py` / `tests/test_export_validation.py`. apiome-rest 1.79.0 → 1.80.0.
