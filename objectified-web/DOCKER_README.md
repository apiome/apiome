# Objectified Web - Docker Setup

This directory contains Docker configuration for the Objectified Web marketing site.
...
docker run -d \
  ...
  -p 3002:3002 \
  objectified-web:latest

# View logs
docker logs -f objectified-web

# Stop and remove
docker stop objectified-web
docker rm objectified-web
```
...
Builds the image as `objectified-web:latest`.
...
DOCKER_REGISTRY=registry.company.com/objectified ./build-docker.sh --push
```
...
```
objectified-web:latest
objectified-web:20260120-143022
```
...
4. Rebuild without cache: `docker build --no-cache -t objectified-web:latest .`
...
```bash
# Pull latest image
docker pull objectified-web:latest
...
- Visit: https://docs.objectified.dev
- Watch tutorials: https://www.youtube.com/@objectifieddev

## 📄 License

Copyright © 2026 Objectified. All rights reserved.
